...
router.post(
  ...
  fetchUser,
  [
    body("QuestionTitle", "Write a suitable title for question").isLength({
      min: 1,
    }),
    body("QuestionDetails", "Explain question in details").isLength({
      min: 20,
    }),
    ...
    if (question) {
      if (question.user.toString() === loggedUserId) {
        ...
        return res
          .status(403)
          .json(
            "cant delete this question as you are not author of this question"
          );
      }
    } else {
      return res.status(404).json("question not found");
...
// route to fetch all question data associated with a user id
router.get("/fetchPostedQuestion", fetchUser, async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(400).json({ errors: errors.array() });
  }
  try {
    ...
    const postedQuestions = await Question.find({ user: userId });
    res.status(200).json(postedQuestions);
  } catch (error) {
    console.error(error);
    return res.status(500).json("Internal server error");
  }
